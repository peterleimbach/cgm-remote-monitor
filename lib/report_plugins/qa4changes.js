...
qa4changes.report = function report_qa4changes(datastorage, sorteddaystoshow, options) {
  ...
  for (var t=0; t<treatments.length; t++) {
    ...
    if (t === 0){
      ...
      minutes = Math.floor((((new Date().getTime() - tr.mills) / 1000 / 60 / 60) - hours) * 60);
    }else {
      hours = Math.floor((lastMills - tr.mills) / 1000 / 60 / 60);
      minutes = Math.floor((((lastMills - tr.mills) / 1000 / 60 / 60) - hours) * 60);
    }
    var bgcolor = '';
    if (hours < 1.25 * 48){
      bgcolor = '#7fff00';
    ...
    table.append($('<tr>').addClass('border_bottom')
      ...
      .append($('<td>').css('background-color' , bgcolor).append(hours + " Stunden " + minutes + " Minuten").append(t === 0 ? " und läuft noch" : "")));
